...
Endpoint `GET /resources/{id}/availability?from=...&to=...` zwraca listę zajętych slotów (rezerwacji w statusie `pending` lub `confirmed`) dla danego zasobu w wybranym przedziale czasowym. To zapytanie jest naturalnym kandydatem na repetowanie: użytkownik scrolluje kalendarz, zmienia widok dnia/tygodnia, wraca do tego samego widoku — za każdym razem strona dopytuje o ten sam zasób w tym samym lub bardzo podobnym oknie czasowym.
...
- TTL: 60 sekund — krótki, ale wystarczający żeby zaabsorbować typowe burst-y kliknięć w UI.
...
**Brak cache, polegamy na partial indexie w bazie.** Index `ix_booking_organization_id` + EXCLUDE USING gist na `(resource_id, tstzrange(...))` z `WHERE status IN ('pending','confirmed')` już jest, więc kwerenda jest szybka. Realistycznie dla mojego MVP byłoby to wystarczające. Wybrałem cache nie dla wydajności, tylko żeby zademonstrować pełną integrację Redisa z aplikacją (element bonusowy z dokumentu wymagań) i jednocześnie mieć ADR o trudnej dyscyplinie "cache invalidation".

**In-memory cache w procesie API** (`functools.lru_cache`, `cachetools`). Najprostsze, zero infrastruktury. Odpada przy >1 worker (każdy worker ma swoją kopię — niespójność), nie przeżywa restartu, i fundamentalnie miesza warstwy: cache to data plane, nie process state.
...
JSON jako format zamiast pickle — czytelne w `redis-cli` przy debugowaniu, bezpieczne (pickle deserializacja to RCE jeśli ktoś coś podstawi do Redisa), wieloplatformowe.
...
**`SCAN` invalidacja jest niedeterministyczna.** Jeśli między `SCAN` a `DEL` ktoś doda nowy klucz pasujący do prefiksu, nie zostanie usunięty. Akceptowalne — TTL go i tak zje za chwilę. W produkcji można by używać Redis sets do śledzenia kluczy per tenant, ale to overengineering dla MVP.
...
**Dodatkowa zależność na ścieżce krytycznej zapisu.** `POST /bookings` teraz robi też `SCAN` + `DEL` w Redisie. Jeśli Redis padnie, request jako całość się nie wywali (Redis client rzuci wyjątek po `await session.commit()`), ale user dostanie 500. Można by to zawinąć w try/except i zignorować błąd cache invalidacji — ale wtedy stale data zostaje na cały TTL, co w trakcie awarii Redisa może oznaczać dłuższą niespójność. Świadomie wybieram fail-fast: lepsze 500 i retry niż udawanie że wszystko OK.
